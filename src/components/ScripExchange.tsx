      ...
      <div className={`root app-root-with-header${darkMode ? ' dark' : ''}`}> 
        <div className={`card${darkMode ? ' dark' : ''}`}> 
          ...
          <div className="controls"> 
            <select
              value={scripType}
              onChange={e => setScripType(e.target.value)}
              className={`select${darkMode ? ' dark' : ''}`}
            >
              {SCRIP_TYPES.map(type => (
                <option key={type} value={type} style={{ fontWeight: 600 }}>
                  {type}
                </option>
              ))}
            </select>
            <button
              ...
              style={{ marginLeft: 12, padding: "0.7rem 1.2rem", fontSize: "1.08rem", borderRadius: 10, fontWeight: 600, border: "none", background: darkMode ? "#6366f1" : "#6366f1", color: "#fff", boxShadow: "0 2px 8px #e0e7ff", cursor: "pointer" }}
